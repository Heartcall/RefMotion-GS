...
## Current Next Action
Rerun the Milestone 3.2 pre-implementation authorization audit after the `PHASE3_PLAN.md` planning repair.
...
## Model Gate
Satisfied if the user command includes explicit operator confirmation that Codex has been switched to the required model.
...
## Success Criteria
- The audit determines whether the repaired Milestone 3.2 dense / tangent-space normal optimization subplan is sufficiently specified, in scope, testable, and safe to implement.
- The audit does not require Milestone 3.2 results yet.
- The audit writes or updates `refmotion_gs_mvp/outputs/phase3/milestone_32_preimplementation_audit.md`.
- The audit updates `DECISION_LOG.md`.
- The audit updates `IMPLEMENTATION_LOG.md`.
- The audit updates `NEXT_ACTION.md`.

## Stop Conditions
- If the user command does not include explicit operator confirmation that Codex has been switched to the required model, stop and request GPT-5.5 xhigh plus model confirmation.
